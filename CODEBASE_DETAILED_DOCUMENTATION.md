...
- **Parallel fetch node** (`parallel_fetch_node`)
  - Fan-out via `asyncio.gather`:
    - registration status
    - policy requirements
    - approvals snapshot
    - eval snapshot
    - artifacts status (for accurate gap analysis)
...
- **Artifact generation node** (`artifact_generation_node`)
  - Generates requested artifacts (dummy generation in this repo) and stores them in state as `_generated_artifacts` so the service layer can persist them.
...
- **Tool calling / governance system orchestration**
  ...
  - Node fanout: `src/uca_orchestrator/orchestrator/nodes.py` (`parallel_fetch_node`)
- **Parallel execution**
  - `src/uca_orchestrator/orchestrator/nodes.py` (`asyncio.gather` in `parallel_fetch_node`)
- **Gap analysis**
...
- **Audit trace**
  - State-level: `audit_log` appended in nodes (`_append_audit`)
...
- `PARALLEL_FETCH` (`parallel_fetch_node`)
  - fan-out tool calls via `InternalApiClient`:
    - registration status
    - policy requirements
    - approvals status
    - eval status
...
- if missing → `ARTIFACT_GENERATION` (`artifact_generation_node`)
  - generates artifacts into state (`_generated_artifacts`)
